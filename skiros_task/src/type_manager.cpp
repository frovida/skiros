...
*/
bool Type_Manager::addType(std::string s){
    if(s == "object"){
...
void Type_Manager::setTypeHeirarchy(){
...
PDDL_Type Type_Manager::getType(std::string s){
    PDDL_Type ret;
    for(PDDL_Type t : types){
        if(t.type_name == s){
            ret = t;
...
void Type_Manager::setSuperType(PDDL_Type type_to_set, std::string current_type){
...
Element Type_Manager::getRack(Element e, int depth){
...
bool Type_Manager::containsNoUsefulParts(Element e){
...
void Type_Manager::findWorldObjects(){
...
std::vector<std::string> Type_Manager::getAllObjects(std::string type){
...
void Type_Manager::addPredicate(std::string p_name, std::vector<std::string> p_args, bool in_skiros){
    PDDL_Predicate p;
...
bool Type_Manager::hasPredicate(std::string pred_name){
    for(PDDL_Predicate p : predicates){
...
std::string Type_Manager::getTypesPDDL(){
    std::stringstream ss;
    ss << "(:types\n\t";
    //top level (object)
...
std::string Type_Manager::getObjectsPDDL(){
    std::stringstream ss;
    ss << "(:objects\n";
    for(PDDL_Type t : types){
...
std::string Type_Manager::getPredicatesPDDL(){
    std::stringstream ss;
    ss << "(:predicates\n";
    for(PDDL_Predicate p : predicates){
        ss << "\t" << p.to_pddl() << "\n";
...
std::string Type_Manager::getInitialStatePDDL(){
    std::stringstream ss;
    ss << "(:init\n";
...
std::string Type_Manager::getGoalPDDL(){
    std::stringstream ss;
    ss << "(:goal (and\n";
...
GroundPredicate Type_Manager::parseGroundPredicate(Element e){
    GroundPredicate gp;
...
void Type_Manager::addObjectByType(std::string type_name, std::string object_name){
    for(unsigned i = 0; i < types.size(); ++i){
...
void Type_Manager::addGroundPredicate(std::string name, std::vector<std::string> args){
    GroundPredicate gp;
...
void Type_Manager::setGoal(std::vector<skiros_wm::Element> conditions, std::vector<std::string> pddl_goals){
...
void Type_Manager::addHiddenObjects(HiddenPropertyElement hpe){
