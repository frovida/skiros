...
void PddlProblem::initDomain()
{
    ...
    for(unsigned i = 0; i < skills.size(); ++i){
        ...
        //Add can_x predicate
        type_manager.addPredicate("can_" + s.skill_name, {robotParameterName}, false);
    ...
    //For abstract types.
    if(STAMINA){
        type_manager.addPredicate("isOfType", {hasAbstractTypes, hasAbstractTypes}, false);
    }
}

void PddlProblem::initProblem()
{
    if(getWorldHandle()==NULL) return;

    ROS_INFO("Getting Objects");
    type_manager.findWorldObjects(); //get all objects of each type
}


void PddlProblem::setGoal(std::vector<skiros_wm::Element> conditions, std::vector<std::string> pddl_goals)
{
    type_manager.setGoal(conditions, pddl_goals);
}

void PddlProblem::outputPDDL(){
    printDomainFile();
    printProblemFile();
}

void PddlProblem::parseSkill(Element skill)
{
...
UngroundPredicate PddlProblem::parseUngroundPredicate(Element e)
{
    UngroundPredicate up;
...
std::string PddlProblem::addPredicate(Element e){
    //Only add if doesn't already exist
    if(type_manager.hasPredicate(e.type()))
        return e.type();
    std::vector<std::string> args;
...
void PddlProblem::printDomainFile(){
    ...
    domainFile << type_manager.getTypesPDDL();
    domainFile << type_manager.getPredicatesPDDL();
    if(STAMINA){
...
void PddlProblem::printProblemFile(){
    ...
    problemFile << type_manager.getObjectsPDDL();
    problemFile << type_manager.getInitialStatePDDL();
    problemFile << type_manager.getGoalPDDL();
    ROS_INFO_STREAM("GOAL: " << type_manager.getGoalPDDL());
...
std::vector<Element> PddlProblem::callPlanner()
{
    plan.clear();
    ...
    if(!use_modified_TFD)
        ROS_INFO("EDIT PddlProblem::callPlanner() to add call to external planner");
...
void PddlProblem::dumpAll(){
    std::cout << type_manager.getTypesPDDL() << std::endl;
    std::cout << type_manager.getPredicatesPDDL() << std::endl;
    if(skills.size() == 0)
        std::cout << "Skills List: Empty" << std::endl;
    else{
        for(PM_Skill s : skills){
            std::cout << s.to_pddl() << std::endl;
        }
    }
    std::cout << type_manager.getObjectsPDDL() << std::endl;
    std::cout << type_manager.getInitialStatePDDL() << std::endl;
    std::cout << type_manager.getGoalPDDL() << std::endl;
}


/*helper functions*/
std::vector<std::string> &PddlProblem::split(const std::string &s, char delim, std::vector<std::string> &elems) {
...
std::vector<std::string> PddlProblem::split(const std::string &s, char delim) {
...
bool PddlProblem::isSpatialRelation(UngroundPredicate up){
...
bool PddlProblem::isRelatedSpatialRelation(UngroundPredicate up1, UngroundPredicate up2){
