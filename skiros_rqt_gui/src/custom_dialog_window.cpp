...
#include "skiros_rqt_gui/interactive_markers.h"
  ...
  : QDialog(parent), parent_(parent)
{
  ...
  int id = qRegisterMetaType<visualization_msgs::InteractiveMarkerFeedbackConstPtr>("InteractiveMarkerMsg");
}
...
int CustomDialog::addSkirosPropertyComboBox(skiros_common::Param & property, QString tooltip)
{
    property_ = &property;
    DialogElement &e = addNewElement(DLG_COMBOBOX, "Property", tooltip, true);
    e.returnInt = NULL;
    e.cmbBox = new QComboBox(this);
    prop_combo_box_ = e.cmbBox;
    std::list<std::string> query_list;
    query_list.push_back("PhysicalObjectProperty");
    while(query_list.size())
    {
        std::stringstream ss;
        for(auto obj_type : query_list)
            ss << wm_ptr_->queryOntology("SELECT ?x WHERE { ?x rdfs:subPropertyOf stmn:" + obj_type + ". } ");
        query_list.clear();
        std::string temp;
        ss >> temp;
        while(!ss.eof())
...
int CustomDialog::addSkirosIndividualComboBox(QString tooltip)
{
  DialogElement &e = addNewElement(DLG_COMBOBOX, "Individual", tooltip, true);
  e.returnInt = NULL;
  e.cmbBox = new QComboBox(this);
  individual_combo_box_ = e.cmbBox;
  connect(individual_combo_box_, SIGNAL(currentIndexChanged(int)), this, SLOT(skirosIndividualCbIndexChanged(int)));
...
int CustomDialog::addSkirosTypeComboBox(QString tooltip)
{
  DialogElement &e = addNewElement(DLG_COMBOBOX, "Type", tooltip, true);
  e.returnInt = NULL;
  e.cmbBox = new QComboBox(this);
  type_combo_box_ = e.cmbBox;
  std::list<std::string> query_list;
  query_list.push_back("Spatial");
  while(query_list.size())
  {
      std::stringstream ss;
      for(auto obj_type : query_list)
  ...
  e.layout->addWidget(e.cmbBox);
  layoutNextElement->addLayout(e.layout);
  addSkirosIndividualComboBox();
  connect(type_combo_box_, SIGNAL(currentIndexChanged(int)), this, SLOT(skirosTypeCbIndexChanged(int)));
  return elements.size();
}

void CustomDialog::skirosTypeCbIndexChanged(int index)
{
    ...
    if(type!="")
    {
        ...
        while(!ss.eof())
        {
            if(temp!="")
            {
                individual_combo_box_->addItem(temp.c_str());
            }
            ss >> temp;
        }
    }
...
void CustomDialog::skirosIndividualCbIndexChanged(int index)
{
    std::string individual = individual_combo_box_->itemText(index).toStdString();
    if(individual=="")
        return;
    skiros_wm::Element e;
    if(individual.find('-')!=std::string::npos)
        e = wm_ptr_->getElementFromUri(individual);
    else
        e = wm_ptr_->getDefaultElement(individual);
    e.id() = -1;
    *linked_e_ = e;
    /*while(elements.size())
    {
    ...
    addSkirosElementEdit(*linked_e_);*/
}

void CustomDialog::skirosPropertyCbIndexChanged(int index)
{
    std::string property_name = prop_combo_box_->itemText(index).toStdString();
    if(property_name=="")
        return;
    skiros_config::ParamTypes p = skiros_config::ParamTypes::getInstance();
    if(p.isStdParam(property_name))
        *property_ = p.getDefault(property_name);
    else
        *property_ = skiros_common::Param(property_name, "", typeid(std::string), skiros_common::symbolic, 1);
}

int CustomDialog::addSkirosElementCreation(skiros_wm::Element &e)
{
    linked_e_ = &e;
...
int CustomDialog::addElementProperties()
{
    ...
    connect(this, SIGNAL(interactiveMarkerFeedback(const visualization_msgs::InteractiveMarkerFeedbackConstPtr &)),
            this, SLOT(interactiveMarkerFeedbackProcess(const visualization_msgs::InteractiveMarkerFeedbackConstPtr &)));
...
{
    skiros_common::Param property;
    CustomDialog d("Add property", parent_);
    d.initSkirosInterfaces(wm_ptr_, interactive_m_server_);
    d.addSkirosPropertyComboBox(property);
    d.exec();
    if(d.wasCancelled())
        return;
    if(linked_e_->addProperty(property))
    {
        std::stringstream ss;
        ss.str("");
        ss << property.key() << ":  ";
...
void CustomDialog::interactiveMarkerFeedbackProcess(const visualization_msgs::InteractiveMarkerFeedbackConstPtr &feedback)
{
    switch ( feedback->event_type )
    {
      ...
      case visualization_msgs::InteractiveMarkerFeedback::POSE_UPDATE:
        if(linked_properties_map_.find("Position")!=linked_properties_map_.end())
        {
            std::stringstream ss;
            ...
            linked_properties_map_["Position"] = ss.str();
        }
        if(linked_properties_map_.find("Orientation")!=linked_properties_map_.end())
        {
            std::stringstream ss;
            ss << std::setprecision(2) << "["
            ...
            linked_properties_map_["Orientation"] = ss.str();
        }
        break;
...
void CustomDialog::updateDisplay()
{
  for(int i = 0; i<(int)elements.size(); i++)
  {
    ...
    if(e.extraChkAdded)
      e.chkExtra->setChecked(*e.returnChkExtra);
    ...
    case(DLG_LINEEDIT):
      e.lineEdit->setText(e.returnString->c_str());
      break;
